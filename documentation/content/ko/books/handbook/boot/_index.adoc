...
첫 번째 단계인 [.filename]#boot1#은 512바이트 크기만 가능하기 때문에 매우 단순합니다. 이것은 슬라이스에 대한 정보를 저장하는 FreeBSD _bsdlabel_과 관련한 정보만 알고 있으며 [.filename]#boot2#을 찾아 실행할 수 있게 해줍니다.
...
초기 시스템 시작 시 부트 man:loader[8]는 man:device.hints[5]를 읽습니다. 이 파일은 “장치 힌트”라고도 하는 변수로 알려진 커널 부팅 정보를 저장합니다. 이러한 “장치 힌트”는 장치 드라이버에서 장치 구성을 위해 사용됩니다.

장치 힌트는 <<boot-loader>>에 표시된 것처럼 세번째 단계의 부트 로더 프롬프트에서 지정할 수도 있습니다. 변수는 `set`을 사용하여 추가하고, `unset`으로 제거하며, `show`로 확인할 수 있습니다. [.filename]#/boot/device.hints#에 설정된 변수는 재정의할 수도 있습니다. 부트 로더에서 입력한 장치 힌트는 영구적이지 않으며 다음 재부팅시 적용되지 않습니다.
