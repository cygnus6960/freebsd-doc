...
언어 코드_ 및 _국가 코드_는 국가 및 특정 언어 변형을 결정하는 데 사용됩니다. <<locale-lang-country>>에서는 __언어코드_국가코드__의 몇 가지 예를 제공합니다:
...
이번 최소예시에서는 개별 사용자의 홈 디렉터리인 [.filename]#.login_conf#에서 Latin-1 인코딩을 위해 두 변수를 모두 설정합니다:
...
기존 사용자의 로그인 클래스를 변경하려면 `chpass`를 사용할 수 있습니다. 슈퍼유저로 호출하고 수정할 사용자 이름을 인수로 제공합니다.
...
X 입력 방법(XIM) 프로토콜은 영어가 아닌 문자를 입력하기 위한 Xorg 표준입니다. <<locale-xim>>은 FreeBSD 포트 컬렉션에서 사용할 수 있는 입력용 애플리케이션을 설명하고 있습니다. 추가적인 Fcitx와 Uim 응용 프로그램도 사용할 수 있습니다.
