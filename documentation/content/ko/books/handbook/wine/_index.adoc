...
바이너리 패키지를 사용하여 FreeBSD에 `winetricks`를 설치하려면 다음 명령을 사용합니다(`와인트릭스`에는 i386-wine 또는 i386-wine-devel 패키지가 필요하나, 다른 종속성 패키지와 함께 자동으로 설치되지 않음에 유의하세요):
...
새 프로그램을 실행하려면 목록에서 해당 프로그램을 선택하고 _OK_를 클릭합니다. 애플리케이션을 제거하려면 메인 화면에서 _Uninstallation_를 선택하면 비슷한 목록이 표시됩니다. 제거할 프로그램을 선택하고 _OK_를 클릭합니다.
...
로그인 시 또는 기본 그래픽 환경이 시작될 때 실행되는 명령 목록에 ㄷ음 명령을 추가해야 합니다. 위의 모든 작업이 완료되면 [.filename]#sudoers#에서 `WINDOW_USERS` 중 하나로 구성된 사용자는 다음 명령과 함께 공유 접두사를 사용하여 프로그램을 실행할 수 있습니다:
...
운영 체제 드라이버는 응용 프로그램과 하드웨어 간에 명령을 전송합니다. 와인은 드라이버를 포함한 Windows(R) 환경을 에뮬레이션하며, 이 전송을 위해 FreeBSD의 기본 드라이버를 사용합니다. 와인 시스템은 호스트 시스템 드라이버를 사용하도록 설계되었으므로 Windows(R) 드라이버를 설치하는 것은 바람직하지 않습니다. 예를 들어 전용 드라이버를 사용하는 그래픽 카드의 경우, Windows(R) 설치 프로그램이 아닌 표준 FreeBSD 방법을 사용하여 설치하십시오.
