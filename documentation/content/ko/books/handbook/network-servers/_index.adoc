...
이 파일을 편잡하고 저장한 후, 시스템 부팅시 inetd가 시작되도록 [.filename]#/etc/rc.conf#을 설정합니다:
...
일반적으로 애플리케이션의 기본 항목은 `+#+`를 제거하는 것 외에는 편집할 필요가 없습니다. 하지만 어떤 경우에는 기본 항목을 편집하는 것이 적절할 수 있습니다.
...
네임 서버는 일반적으로 권한 있는 네임 서버와 캐싱(리존버라고도 함) 네임 서버의 두 가지 형태로 제공됩니다.
...
[[network-iscsi]]
== iSCSI 초기자(Initiator)와 타겟 구성

iSCSI는 네트워크를 통해 스토리지를 공유하는 방법입니다. 파일 시스템 수준에서 작동하는 NFS와 달리 iSCSI는 블록 장치 수준에서 작동합니다.

iSCSI 용어로는 스토리지 공유를 제공하는 시스템을 _타겟_이라고 합니다. 스토리지는 물리적 디스크일 수도 있고, 여러 디스크 또는 물리적 디스크의 일부를 나타내는 영역일 수도 있습니다. 예를 들어, 디스크가 ZFS로 포맷된 경우 zvol을 생성하여 iSCSI 스토리지로 사용할 수 있습니다.
...
FreeBSD는 기본 커널 기반 iSCSI 타겟과 초기자를 제공합니다. 이 섹션에서는 FreeBSD 시스템을 타겟 또는 초기자로 구성하는 방법을 설명합니다.

[[network-iscsi-target]]
=== iSCSI 타겟 구성하기

iSCSI 타겟을 구성하려면 [.filename]#/etc/ctl.conf# 구성 파일을 만들고 [.filename]#/etc/rc.conf#에 man:ctld[8] 데몬이 부팅 시 자동으로 시작되도록 하는 명령을 추가한 다음 데몬을 시작하세요.
...
두 번째 항목은 단일 타겟을 정의합니다. 타겟은 두 가지 의미로 사용할 수 있습니다. iSCSI를 제공하는 컴퓨터 또는 명명된 LUN 그룹입니다. 이 예에서는 후자의 의미를 사용하며, 여기서 `iqn.2012-06.com.example:target0`은 타겟 이름입니다. 이 타겟 이름은 테스트 목적으로 적합합니다. 실제 사용하려면 `com.example`을 실제 도메인 이름으로 변경합니다. `2012-06`은 해당 도메인 네임의 제어권을 획득한 연도와 월을 나타내며, `target0`은 어떤 값이라도 사용할 수 있습니다. 이 구성 파일에는 원하는 수의 타겟을 정의할 수 있습니다.

`auth-group no-authentication`줄은, 모든 초기자가 지정된 타겟에 연결할 수 있도록 허용하고 `portal-group pg0`은 `pg0` 포털 그룹을 통해 타겟에 연결할 수 있도록 합니다.

다음 섹션에서는 LUN을 정의합니다. 초기자에게는 각 LUN이 별도의 디스크 장치로 표시됩니다. 각각의 타겟에 대해 여러 개의 LUN을 정의할 수 있습니다. 각 LUN은 숫자로 식별되며, 여기서 LUN 0은 필수입니다. `path /data/target0-0` 줄은 LUN을 백업한 파일 또는 zvol의 전체 경로를 정의합니다. 이 경로는 man:ctld[8]을 시작하기 전에 반드시 있어야 합니다. 두 번째 줄은 선택 사항이며 LUN의 크기를 지정합니다.
...
man:ctld[8] 데몬이 시작되면 [.filename]#/etc/ctl.conf#을 읽습니다. 데몬이 시작된 후 이 파일을 편집하는 경우 다음 명령을 사용하면 변경 사항이 즉시 적용됩니다:
...
`auth-group` 섹션은 사용자 이름과 암호 쌍을 정의합니다. `iqn.2012-06.com.example:target0`에 연결하려는 초기자는 먼저 정의된 사용자 이름과 암호를 제공해야 합니다. 그러나 단순 타겟 검색은 인증 없이도 허용됩니다. 타겟 검색에도 인증을 요구하려면 `discovery-auth-group`을 `no-authentication` 대신 정의된 `auth-group` 이름으로 설정합니다.

모든 초기자에 대해 하나의 타겟만 내보내는 것이 일반적입니다. 위의 구문을 줄여서 사용자 이름과 비밀번호를 타겟 항목에 직접 지정할 수 있습니다:
...
타겟에 연결할 때 [.filename]#/etc/iscsi.conf# 구성 파일을 사용하거나 사용하지 않고 연결할 수 있습니다. 이 섹션에서는 두 가지 유형의 연결을 모두 보여줍니다.

==== 구성파일 없이 타겟에 연결하기

초기자를 단일 대상에 연결하려면 포털의 IP 주소와 타겟의 이름을 지정합니다:
...
이 예제에서는 [.filename]#/dev/da0#이 연결된 LUN을 나타내며 iSCSI 세션이 성공적으로 설정되었습니다. `iqn.2012-06.com.example:target0` 타겟이 둘 이상의 LUN을 내보내는 경우 출력의 해당 섹션에 여러 장치 노드가 표시됩니다:
...
이 메시지는 지정한 타겟 이름이 잘못되었다는 의미입니다:
...
지정된 타겟에만 연결하기 위해 정확한 별명을 지정해 주세요:
...
구성파일에 있는 모든 타겟에 연결하기 위해서는 다음을 사용하세요:
...
초기자가 [.filename]#/etc/iscsi.conf#의 모든 타겟에 자동으로 연결되도록 하려면 [.filename]#/etc/rc.conf#에 다음을 추가합니다:
